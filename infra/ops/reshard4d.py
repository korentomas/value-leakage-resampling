...
for i, (name, port) in enumerate(shards):
    ...
    cmd = (f'cd {R}/swap && {PY} driver.py run --requests {p} --results {R}/artifacts/step4d_results_{name}.jsonl '
           f'--base-url http://localhost:{port} --concurrency 64 --timeout 3600 > {R}/infra/ops/logs/step4d-run-{name}.log 2>&1; '
           f'echo "4D_EXIT={name}=$?" >> {R}/infra/ops/logs/step4d-run-{name}.log')
