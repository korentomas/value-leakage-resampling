...
$PY - <<'PYEOF'
import json
import os; R=os.path.join(os.getcwd(),'..','artifacts')
...
PYEOF
$PM model.py --counts $R/artifacts/step4g_counts_2arm.jsonl --prior all --informed-dep0 0.27 --seed 0 --out $R/artifacts/step4g_summaries.json --netcdf $R/artifacts/step4g_idata.nc > $R/infra/ops/logs/step4g-fit-dep.log 2>&1
mkdir -p $R/artifacts/step4g_fits
$PM three_arm.py --step3-counts $R/artifacts/step4g_counts_2arm.jsonl --neutral-counts $R/artifacts/step4g_counts_neutral.jsonl --baseline-counts $R/artifacts/step4g_baseline_counts.jsonl --step3-summaries $R/artifacts/step4g_summaries.json --pairs r --prior all --informed-dep0 "r=0.135,c=0.135,s=0" --out-dir $R/artifacts/step4g_fits --tag step4g > $R/infra/ops/logs/step4g-fit-r.log 2>&1
echo FIT4G_DONE
