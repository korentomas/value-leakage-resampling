...
*Figure 6. (a) Where the answer locks and where the first stated intent appears, as fractions of the CoT on the same scale as the cuts. (b) The two subtracted per conversation, which the overlapping distributions in (a) can't show. Mass right of zero is a conversation whose intent came after its answer was already settled, and that is 81% of them.*
