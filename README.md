...
Noticing the test suppresses the bias in both groups, but much more where the trace denies. Denying prefixes go from +0.30 to +0.17 when the model notices, admitting ones only from +0.33 to +0.26. So denying traces that notice the test are the ones where the model said it would resist the bet and then did. There's a catch. Both labels are read off the same text, so I can't rule out that they're keying on the same sentence.
