...
Captions live in `paper.tex` (on the `v2-writeup-and-figures` branch), not here, because in the paper the caption carries
...
qualifying clause, which is the parallel-triple pattern `tools/slopcheck.py` (on the `v2-writeup-and-figures` branch)
