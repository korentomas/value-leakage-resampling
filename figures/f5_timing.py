...
fig, (ax, axb) = plt.subplots(2, 1, figsize=(9, 9.2), sharex=True,
...
ax.scatter(first, ys, s=16, color=C_CLAIM, zorder=3, lw=0)
ax.scatter(lock_d, ys, s=16, color=C_LOCK, zorder=4, lw=0)

for x, col, lab, dy in ((np.median(lock_d), C_LOCK, "lock", 2.2), (np.median(first), C_CLAIM, "first claim", 0.2)):
    ax.plot([x, x], [-1.5, n + 0.5], color=col, lw=0.8, ls=(0, (3, 3)), zorder=2)
ax.text(np.median(lock_d) + 0.012, n + 1.2, f"median lock {np.median(lock_d):.2f}", color=C_LOCK, fontsize=9, ha="left", va="bottom")
ax.text(np.median(first) - 0.012, n + 1.2, f"median first claim {np.median(first):.2f}", color=C_CLAIM, fontsize=9, ha="right", va="bottom")

ax.set_ylim(-1.5, n + 7)
ax.set_yticks([])
ax.spines["left"].set_visible(False)
ax.set_ylabel(f"{n} denying conversations, sorted by lock", labelpad=6)
ax.legend(handles=[
    Line2D([], [], marker="o", ls="", color=C_LOCK, ms=5, label="answer locked (posterior median)"),
    Line2D([], [], marker="o", ls="", color=C_CLAIM, ms=5, label="first honesty claim"),
    Line2D([], [], color=style.FAINT_STEERED, lw=1.6, label=f"claim written after the lock  ({after.mean():.0%})"),
    Line2D([], [], color=style.FAINT, lw=1.6, label=f"claim written before the lock  ({1 - after.mean():.0%})"),
], loc="lower center", bbox_to_anchor=(0.5, 1.0), ncol=2, fontsize=9, columnspacing=1.8)
...
ax_j = 0.16
axb.scatter(lock_i, y_lock + rng.uniform(-ax_j, ax_j, m), s=12, color=C_LOCK, alpha=0.7, lw=0, zorder=3)
axb.scatter(ipos, y_int + rng.uniform(-ax_j, ax_j, m), s=12, color=C_INTENT, alpha=0.75, lw=0, zorder=3)
for y, x, col in ((y_lock, np.median(lock_i), C_LOCK), (y_int, np.median(ipos), C_INTENT)):
    axb.plot([x, x], [y - 0.3, y + 0.3], color=col, lw=2.2, zorder=4)
    axb.text(x, y + 0.34, f"{x:.2f}", ha="center", va="bottom", fontsize=9, color=col)
axb.set_yticks([y_lock, y_int])
axb.set_yticklabels(["lock", "first intent\nstatement"], fontsize=9.5)
axb.tick_params(axis="y", length=0)
axb.spines["left"].set_visible(False)
axb.set_ylim(-0.55, 1.75)
axb.text(1.0, 1.62, f"{m} conversations with an explicit intent to fall on the good side;\n"
                    f"intent written after the lock in {np.mean(ipos > lock_i):.0%}",
         ha="right", va="top", fontsize=9, color=style.INK, linespacing=1.25)
