...
Both read the model's own variability under a single prompt. The step taken here
is to resample under two prompts and compare them, so that where those methods
ask how undecided the model still is, this asks what the remaining indecision
depends on. In Bigelow et al.'s notation the quantity defined below is a
distance between two $o_t$ curves produced under different prompts, so theirs is
an observational measurement and mine an interventional one.
