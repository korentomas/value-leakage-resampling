...
(Fig.~\ref{fig:threearm}). The text has inherited the bias. Mean $s(0.2)$ is $+0.27$ [0.25, 0.30], which
doubled onto the paper's scale is 0.55 [0.49, 0.60], against the 0.62 those same
rollouts show at $t=0$, so 88\% [80\%, 97\%] of the bias is already in the
text. Most prefixes this early contain no estimate at all. The bet determines the
...
[$-0.07$, $+0.01$], which crosses zero but points the same way. Deniers and admitters are not
equally aware either, 53\% against 61\%, so the unsplit contrast partly reflects
that imbalance. Taking the difference within each stratum and averaging the two
by stratum size gives $-0.13$ [$-0.16$, $-0.09$] on the second measure, against
$-0.12$ [$-0.16$, $-0.08$] unadjusted, so the result is not an artefact of the
split. Noticing the test suppresses the bias in both
...
would resist the bet and then did. This split was not planned before the runs, and is
reported as a covariate rather than as a finding in its own right.
...
in 8\% of samples and in none of the 85 denying conversations. Whether it says yes tracks what its own chain of thought said. The correlation between a conversation's yes-rate and its $s(0.2)$ is
$-0.07$ [$-0.22$, $+0.08$].
...
Of the 250 conversations, 147 state at some point which side they mean to land
on. Admitting traces do so more often than denying ones, 68\% against 44\%, and
when a side is named it is usually the favoured one: 103 of the 147 point that
way, 12 the other way, and in the remaining 32 the direction is not clear from
the sentence. In those 147, the statement comes after the side is settled in 4
cases out of 5 (Fig.~\ref{fig:timing}). Honesty claims are a separate
measurement and behave differently. Sentences such as ``I must be accurate''
could be located in 77 of the denying conversations, at a median position of
0.23 against a median lock of 0.26, with only 45\% falling after the lock (Fig.~\ref{fig:honesty}). They
arrive with the decision rather than following it, and the traces that make them
still lean toward the favoured side, with mean $s$ of $+0.23$ against $0.00$ for
prefixes written with no bet.
...
chain of thought on the same scale as the cuts. The lock has median 0.29
and the first stated intent 0.63. (b) The two subtracted within each
conversation, which the overlapping distributions in (a) cannot show. A positive
difference means the stated intent came after the answer was settled, which
holds for 81\% of the 147. The lock is
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/out/paper/F7_honesty.pdf}
\caption{Position of the lock and of the first honesty claim, for the 77
denying conversations in which a claim could be located. (a) The two
distributions, as fractions of the chain of thought on the same scale as the
cuts. The lock has median 0.26 and the first claim 0.23. (b) The two subtracted
within each conversation. A positive difference means the claim came after the
answer was settled, which holds for 45\% of them. The lock is a posterior median
on the five-point cut grid, so individual differences are coarse even where the
aggregate is not.}
\label{fig:honesty}
\end{figure}
...
Qwen3.6-35B scores 0.27 in the original paper, against Qwen3.5's 0.62. I ran a smaller version of the
same design on it: 100 conversations, cut at $t = 0.2$ and $1.0$ only, for
12,500 continuations. $\mathrm{dep}(0)$ comes out at 0.27, matching the
published figure.

$\mathrm{dep}$ falls from 0.27 to 0.10 by $t = 0.2$ and reaches 0 by the end,
so the answer settles early here too. Mean $s(0.2)$ is $+0.10$ [$+0.04$, $+0.16$]. Doubled onto the paper's scale
that is 0.20, and their published bias for this model is 0.27, so 74\% of it is
already in the text at that cut, against 88\% for Qwen3.5. Denying traces lean less than
admitting ones again, $+0.03$ against $+0.16$, a difference of $-0.13$
[$-0.26$, $+0.02$]. That is the same sign as Qwen3.5's $-0.06$, but with only 20
admitters the interval covers zero.

One thing differs. Qwen3.6's bias is almost entirely a below-good effect: at
$t = 0.2$, $s$ is $+0.01$ in the above-good version of the question and $+0.20$
in below-good, where Qwen3.5 leans in both. This matches the trajectories in
Betley et al.'s Figure 35.

I did not run the no-bet baseline or the self-report for this model.
...
most such prefixes contain no estimate and the monitor would be reading framing, with no arithmetic there yet. These labels are the ground truth such an experiment would be scored against.
...
Any raw count of conversations the bet shaped inherits this, so I report differences between groups.

\subsection{Sample size per cell}

Twenty-five samples per cell cannot resolve a difference below about 0.3, so every per-conversation number is a posterior.
