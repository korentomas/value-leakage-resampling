...
truncated at $t = 0.21$ and the suffix discarded. The retained prefix,
byte-identical across conditions, is continued $S = 25$ times under the
...
favours is printed beside each row. $\mathrm{dep}$ is the original arm minus the
swapped arm; $s$ is the no-bet arm minus one half.}
\label{fig:method}
\end{figure*}
...
\caption{Dependence on the bet, $\mathrm{dep}(t)$, for 250 conversations
(Qwen3.5-35B; Donation Bet). (a) Mean over conversations with the 95\% spread;
the dashed line is $\mathrm{dep}=0$. $\mathrm{dep}(0)=0.62$ recovers the
published population bias, and the mean falls to 0.26 by $t=0.2$. (b) One row
per conversation, sorted by lock position, with colour giving posterior mean
$\mathrm{dep}$ at each of the five measured cuts. The mean in (a) hides
heterogeneity: most conversations go dark immediately after $t=0.2$, a minority
stay dependent on the bet out to $t=1$. These rows are the per-conversation
labels the method produces. (c) Distribution of lock position over the same
...
cut the original and no-bet arms coincide, so the bet has stopped acting through
the prompt and the retained text carries the bias. The gap between the no-bet
arm and the baseline is $s(t)$; the gap between the original and swapped arms is
$\mathrm{dep}(t)$. At $t=0.2$, $s=+0.24$ and $\mathrm{dep}=0.33$ above-good,
...
original paper's judge. A conversation counts as bet-shaped if its no-bet
...
admitting ones, the opposite of what selective concealment predicts. Around a
third of no-bet prefixes clear the same bar by chance, which inflates every
group equally and cancels in the contrast printed above the bars.}
\label{fig:denial}
\end{figure}
...
chain of thought; the lock has median 0.29, the first stated intent 0.63. (b)
