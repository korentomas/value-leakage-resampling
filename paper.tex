...
horizontal rule, and the share landing on the side the original prompt favours
is given at the right of each row. The two quantities the method produces are
formed from those shares: $\mathrm{dep}$ is the original arm minus the swapped
arm, and $s$ is the no-bet arm minus one half.}
\label{fig:method}
\end{figure*}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/out/paper/F4_denial.pdf}
...
the swapped prompt can still move it. Bars are posterior means with 95\%
equal-tailed intervals. Denying traces are if anything less bet-shaped than
admitting ones, which is the opposite of what selective concealment predicts.
About a third of no-bet prefixes clear the same bar by chance, which inflates
every group equally and cancels in the contrast quoted above the bars.}
\label{fig:denial}
\end{figure}
