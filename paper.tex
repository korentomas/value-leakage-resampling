...
88\% [80\%, 97\%] of the 0.62 those same rollouts show at $t=0$. Most prefixes this early contain no estimate at all. What the bet decides is
which way the model commits, not whether it commits at all. Count the prefixes
at $t = 0.2$ that lean by more than 0.2, towards or away from the side the bet
makes good. With no bet in the prompt 34\% lean towards that side and 35\% away
from it, so 69\% have already picked a side and split about evenly. With the
bet it is 70\% towards and 7\% away, so 77\% have picked a side. Almost the same
number commit early either way, and what the bet changes is that they stop
splitting evenly.
