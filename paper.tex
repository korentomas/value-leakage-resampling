...
swapped arm, and $s$ is the no-bet arm minus one half.}
\label{fig:method}
\end{figure*}
...
$t = 0.2$, while with the bet those become 70\% and 7\%. Qwen commits early on these questions either way, and the bet decides only which
way.
...
arm and the baseline is $s(t)$, and the gap between the original and swapped
arms is $\mathrm{dep}(t)$. At $t=0.2$, $s=+0.24$ and $\mathrm{dep}=0.33$ above-good,
