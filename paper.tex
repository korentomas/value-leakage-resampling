...
\title{Value Leakage Is Settled Before the Chain of Thought Discusses It}
...
\caption{One conversation, cut mid-thought, continued under three prompts. A
Qwen3.5 chain of thought (crochet question, above-good, threshold
3{,}500{,}000{,}000) is cut at $t = 0.21$; the original ending is discarded and
the retained prefix, byte-identical across all three arms, is continued 25 times
under the original prompt, the no-bet prompt, and the prompt with the two causes
exchanged. Each ending is scored against the model's own no-bet median. Here
$\mathrm{dep} = 0.56$ and $s = +0.29$.}
\label{fig:method}
\end{figure*}
...
Treating the continuation counts as binomial is not merely convenient. Bigelow
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/out/pdf/F2_final.pdf}
\caption{Most of the bet's hold on the answer is gone by $t = 0.2$. Thin lines
are per-conversation posterior mean $\mathrm{dep}(t)$; thick lines are group
means; bands are the interquartile range across conversations in the group,
which is not a posterior interval. Sienna marks the 94 conversations a swapped
prompt can still move by more than 0.2 at some cut $\geq 0.2$ with posterior
probability above 0.9, grey the 156 it cannot. Curves are interpolated through
the five measured cuts and clipped to the range $\mathrm{dep}$ is defined on.}
\label{fig:curves}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/out/pdf/F3_three_arm.pdf}
\caption{Given a prefix, the no-bet prompt and the original prompt land in the
same place. Share of continuations on the original prompt's favoured side, by
cut and condition. The gap between the no-bet arm and the 0.5 baseline is
$s(t)$; the gap between the original and swapped arms is $\mathrm{dep}(t)$.
These are pooled point estimates rather than posteriors, so no band is drawn.}
\label{fig:threearm}
\end{figure*}
...
judge labels this trace as denying influence; its statement of intent, ``I
should not bias low'', arrives at $t = 0.65$, long after the cut.
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/out/pdf/F4_denial.pdf}
\caption{A chain of thought that declares an intention to ignore the bet leans
toward the favoured side slightly less than one that admits the influence.
(a) Posterior share of each group whose no-bet continuations already lean by
more than 0.2, or that a swapped prompt can still move. (b) Denies minus
Admits. Around a third of no-bet prefixes clear the same bar by chance, which
inflates both groups equally, so the difference is the quantity of interest.}
\label{fig:denial}
\end{figure*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/out/pdf/F6_intent_strips.pdf}
\caption{Position of the lock and of the first stated intention to land on the
favoured side, for the 147 traces that contain such a statement; the statement
follows the lock in 81\% of them. The lock is a posterior median on the
five-point cut grid, so its position is coarse, and the figure compares medians
...
On this task, Qwen3.5 fixes which side of the threshold it will land on about a
fifth of the way through its chain of thought, before it writes an estimate. The
text carries most of the population bias from that point on, and the model's own
commentary about the bet arrives at or after it. Denials do not concentrate on
the conversations the bet moved, which is evidence against selective
concealment and leaves absent introspection and trained covertness as the
candidates worth testing. The measurement that supports all of this is a label
per conversation obtained from a model nobody modified, which is the part I
expect to be reusable.
