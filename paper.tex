...
\citet{bigelow2024} introduced Forking Paths Analysis. They start from a single
reasoning chain, which they call the base path, resample it from every position
$t$, and track the resulting outcome distribution $o_t$, so that the points
where the model's answer distribution shifts show up as jumps in $o_t$. Thought Branches
