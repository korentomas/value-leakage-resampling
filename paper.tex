...
two strata back together, the bet-shaped contrast is $-0.13$ [$-0.16$, $-0.09$]
against $-0.12$ [$-0.16$, $-0.08$] unadjusted, so the result is not an artefact
of the split. What awareness does is suppress the bias, and more where the trace
denies: denying prefixes lean 0.30 when the model does not notice the test and
0.17 when it does, while admitting ones move only from 0.33 to 0.26. Denial and
awareness together therefore mark the traces where the model said it would
resist the bet and then did, though that cannot be separated from both labels
being read off the same text. I also asked the model
directly. Each conversation was replayed in full and then asked whether the
