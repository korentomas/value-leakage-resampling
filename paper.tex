...
which way the model commits, not whether it commits at all. Counting the
prefixes that lean by more than 0.2 in either direction at $t = 0.2$: with no
bet in the prompt 34\% lean high and 35\% lean low, so 69\% have already picked
a side and split about evenly between the two. With the bet it is 70\% high and
7\% low, so 77\% have picked a side. Almost the same number commit early either
way, and what changes is that they stop splitting evenly.
